    ...
    async function fetchResults(url: string) {
        ...
        try {
            ...
            if (respose.ok) {
                ...
                setState(state => ({ ...state, status: respose.status, data: result, isLoaded: true, isLoading: false }));
            } else {
                setState(state => ({ ...state, status: respose.status, error: respose.statusText, isLoading: false }))
            }
        } catch (error: unknown) {
